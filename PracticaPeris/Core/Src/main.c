...
int main(void)
{
  ...
  // Periferics info:
  // TIM2	 	-> FreeRunning TIMER per comptar tempts per velocitat
  ...
  // ---
  // ADC1 (C3)	-> Front Sensor
  // ADC2 (C5)	-> Back Sensor
  // ---
  // DAC (A5)	-> DAC per a fer testing dels ADCs
  ...
  ASIM_init(&hdac, DAC_CHANNEL_2);
  SEN_init(&hadc1, &hadc2);
  DVIS_Init();
  /* USER CODE END 2 */
  ...
  while (1)
  {
	  // Fem que s'aturi la CPU mentre no hi hagi una interrupció (Wait For Interrupt)
...
static void MX_LTDC_Init(void)
{
  ...
  hltdc.Init.Backcolor.Blue = 0xF;
  hltdc.Init.Backcolor.Green = 0xF;
  hltdc.Init.Backcolor.Red = 0xF;
  if (HAL_LTDC_Init(&hltdc) != HAL_OK)
  {
    Error_Handler();
  }
  pLayerCfg.WindowX0 = 30;
  pLayerCfg.WindowX1 = 240+30-1;
  pLayerCfg.WindowY0 = 4;
  pLayerCfg.WindowY1 = 320+4-1;
  pLayerCfg.PixelFormat = LTDC_PIXEL_FORMAT_ARGB4444;
  pLayerCfg.Alpha = 0;
  pLayerCfg.Alpha0 = 0xF;
  pLayerCfg.BlendingFactor1 = LTDC_BLENDING_FACTOR1_CA;
  pLayerCfg.BlendingFactor2 = LTDC_BLENDING_FACTOR2_CA;
  pLayerCfg.FBStartAdress = 0xD0000000;
  pLayerCfg.ImageWidth = 240;
  pLayerCfg.ImageHeight = 320;
  pLayerCfg.Backcolor.Blue = 0xF;
  pLayerCfg.Backcolor.Green = 0xF;
  pLayerCfg.Backcolor.Red = 0xF;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
    if (GPIO_Pin == INA_Pin || GPIO_Pin == INB_Pin) ENC_encode_signal(GPIO_Pin);
    else if (GPIO_Pin == INIDX_Pin) BNC_GPIOHandler();
}

// Funcio que s'executa quan la DMA ha fet totes les transferencies
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc) {
	SEN_processADCSamples(hadc->Instance);
}
